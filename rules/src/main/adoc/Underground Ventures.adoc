...
:revnumber: W4056
:revdate: 2014-09-02
//:revremark: Repos dans un donjon. OpÃ©rateurs logiques.
:doctype: article
:lang: fr
:encoding: utf8
:toc:
...
* Cartes : deck de pioche : monstres {lcurl}-}, pièges
{lcurl}-}, trésors {lcurl}{plus}}, améliorations
{lcurl}{plus}} ({lcurl}-}=dos rouge,
{lcurl}{plus}}=dos bleu)
** {plus} 4 cartes porte et/ou escalier par donjon (1 par niveau)
{plus} 1 carte aventurier ({plus}1 pion) par joueur
** on garde l'idée des modules
* 4 Niveaux, n{plus}1 donjons, les portes pourraient être imprimées
sur des feuilles pour économiser des cartes
* 5 cartes en main par joueur {plus} draft
* on révèle les 3 premières carte de la pioche
* à son tour un joueur exécute 2 phases : le dungeon building, puis le
dungeon crawling (ou l'inverse ?)
* Building : le joueur pose autant de cartes qu'il veut sur les donjons
(max 2 par donjon), puis il tire des cartes (cachées ou visibles) pour
compléter
* règles de pose : sur n'importe quel niveau, de gauche à droite,
retournée dos visible
* Crawling : 2 possibilités : repos au village, repos dans un donjon,
aventure dans un donjon
* repos au village : le pion doit être présent devant le joueur au début
du tour, il récupère tous ses curseur au max (vie, pouvoir)
* repos dans un donjon : le pion doit être présent dans un donjon au
début du tour, l'aventurier ne peut rien faire d'autre pendant le tour,
chaque(?) curseur (vie, force, pouvoir) peut remonter d'1(?) unité à
concurrence de son maximum
* aventure au donjon : le joueur enchaîne des actions parmi les
suivantes (il s'arrête quand il veut) : entrée dans un donjon, avancer
sur la carte suivante, revenir au village, passer
* entrée au donjon : le pion doit se trouver au village, choisir un
donjon et poser le pion sur la porte du 1er niveau, il faut qu'il y ait
au moins une carte dans le 1er niveau de ce donjon
* revenir au village : placer le pion au village, le joueur ne peut plus
que passer (il devra attendre le tour suivant s'il veut passer)
* passer : fin de la phase de crawling, c'est au tour du joueur suivant
* avancer sur la carte suivante : il faut qu'il y ait une carte
...
*** l'effet d'une amélioration peut être à usage unique, permanent, ou
immédiat, on remet la carte dans la défausse après utilisation (sauf si
permanent)
** monstre/piège :
*** la première ligne indique la/les condition(s) à remplir par
l'aventurier pour remporter le combat
*** la deuxième ligne indique la/les conséquence(s) pour l'aventurier en
cas de victoire
*** la troisième ligne indique la/les conséquence(s) pour l'aventurier
en cas de défaite
::: Exemple :
:::: [underline]#Condition#: Force >=4
:::: [underline]#Victoire#: Vie - 2, Or {plus} 1
...
*** en cas de victoire, la carte de monstre est récupérée dans
l'équipement de l'aventurier si elle comporte un trésor, elle est
défaussée sinon
*** en cas de défaite, le joueur passe

* lorsqu'il n'y a pas de carte suivante, on peut passer au niveau
suivant, pour autant qu'il y ait au moins une carte sur ce niveau, sinon
fin de donjon et retour au village
* on pourrait imposer l'utilisation d'une échelle pour passer au niveau
suivant, auquel cas les échelles se poseraient face visible et
clôtureraient (ou pas?) un niveau -> à voir pendant les tests
