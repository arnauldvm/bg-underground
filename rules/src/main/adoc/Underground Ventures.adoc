...
:revnumber: W4057
:revdate: 2014-09-02
//:revremark: (Carnet de notes): mise en page, formulations
:doctype: article
:lang: fr
:encoding: utf8
:toc:
...
{lcurl}{plus}}=dos bleu)
** {plus} 4 cartes *porte* et/ou *escalier* par *donjon* (1 par
niveau) {plus} 1 carte *aventurier* ({plus}1 pion) par joueur
** on garde l'idée des *modules* (càd le deck est constitué de plusieurs
paquets de cartes (un par joueur), thématisé (morts-vivants,
peaux vertes, chaos ...) et comprennant le nombre de cartes nécessaires
pour un joueur.
* 4 Niveaux, _n_{plus}1 donjons (si _n_ joueurs), les portes
...
* au début : 5 cartes en main par joueur {plus} draft
...
{minus}> à voir pendant les tests

? Ajouter un 'marché' pour améliorer l'aventurier hors donjon ?
{minus}> uniquement si ça peut résoudre des problèmes d'équilibrage.
